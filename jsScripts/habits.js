...
function calcEcoChanges() {
    ...
    return [...lblDailyEcoTasksTitle, ...state.customTasks]
        .filter(task => set.has(task.id))
        .reduce((sum, task) => sum + task.reduction, 0);
}
...
function updateScreen() {
    ...
    $("#baseScore").textContent = calcBaseEcoScore().toFixed(2);
    saveState();
}